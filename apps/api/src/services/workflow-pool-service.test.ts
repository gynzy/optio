...
vi.mock("./repo-pool-service.js", () => ({
  ...
  parseJsonEnv: (name: string, value: string | undefined) => {
...
describe("getOrCreateWorkflowPod — nodeSelector and tolerations env vars", () => {
  ...
  beforeEach(() => {
    ...
    // Mock the query chain for getOrCreateWorkflowPod:
    // 1. Existing pods query (.where().orderBy())
    (db as any).where.mockReturnValueOnce(thenableRows([]));
    // 2. Count query for pod limit check
    (db as any).where.mockReturnValueOnce(thenableRows([{ count: 0 }]));
    // 3. pickNextInstanceIndex query
    (db as any).where.mockReturnValueOnce(thenableRows([]));
    // 4. Update after create
    (db as any).where.mockResolvedValue([]);

    (db as any).returning.mockResolvedValueOnce([insertedPod]);
  });
  ...
  it("passes parsed nodeSelector to the container spec", async () => {
    ...
    await getOrCreateWorkflowPod("wf-run-1", {});
